...
| **`whiteboard_scene`** | **`WhiteboardScene`** | **`whiteboardElements`** (list of drawing steps) | **`title`, `backgroundColor`, `color` (stroke), `accentColor`** | **Progressive SVG draw-on animation on a whiteboard/paper background. Elements: `text` (typed char-by-char), `line`, `arrow` (with arrowhead), `rect`, `circle`, `path` (arbitrary SVG path), `pause`. Coordinates in px on a 1920×1080 canvas. Ideal for concept diagrams, explainer sketches, math proofs, or architecture overviews.** |

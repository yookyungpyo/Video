#!/usr/bin/env bash
# Soft clay soundtrack for the "Safety" 5-card video (~18.4s, 550f@30fps).
# Cuts (frames 95/200/325/430) = 3.17 / 6.67 / 10.83 / 14.33 s.
# Usage: bash scripts/safety-soundtrack.sh <in.mp4> <out.mp4>
set -euo pipefail
IN="${1:-../projects/cardnews/safety-video.mp4}"; OUT="${2:-../projects/cardnews/safety-video-sound.mp4}"
...
rows=(
 "pad 0 0.22"
 "swish 3170 0.16" "swish 6670 0.16" "swish 10830 0.16" "swish 14330 0.16"
 "pop 320 0.34" "pop 640 0.30"
 "pop 3450 0.34" "pop 6800 0.34" "pop 11000 0.34"
 "pop 14550 0.34" "pop 14920 0.30"
 "twinkle 3300 0.18" "twinkle 6900 0.18" "twinkle 11050 0.18"
 "blip 3900 0.15" "blip 4050 0.15" "blip 4200 0.15"
 "blip 7350 0.15" "blip 7550 0.15" "blip 7750 0.15"
 "blip 11500 0.15" "blip 11650 0.15" "blip 11800 0.15"
)
inp=""; fc=""; lab=""; n=${#rows[@]}
...
fc+="[mx]volume=3.6,acompressor=threshold=-20dB:ratio=3:attack=14:release=200,loudnorm=I=-16:TP=-1.5:LRA=11,alimiter=limit=0.97,lowpass=f=15000,afade=t=out:st=17.8:d=0.5,atrim=0:18.39,aformat=channel_layouts=stereo[out]"

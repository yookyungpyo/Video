...
genf pad "(sin(2*PI*130.81*t)+0.85*sin(2*PI*196*t)+0.7*sin(2*PI*261.63*t)+0.5*sin(2*PI*329.63*t))*0.17*(0.9+0.1*sin(2*PI*0.07*t))" 51.9 "lowpass=f=1500, aecho=0.8:0.9:330:0.3"
# gentle consonant arpeggio (G4 B4 D5 B4)
genf arp "sin(2*PI*(392*eq(mod(floor(t/0.5),4),0)+493.88*eq(mod(floor(t/0.5),4),1)+587.33*eq(mod(floor(t/0.5),4),2)+493.88*eq(mod(floor(t/0.5),4),3))*t)*exp(-4.8*mod(t,0.5))*0.5" 51.4 "lowpass=f=2600, aecho=0.8:0.85:250:0.3"
# soft low heartbeat pulse
genf sub "sin(2*PI*65.41*t)*exp(-6*mod(t,0.6))*0.9" 51.4 "lowpass=f=150"
...
rows=(
 ...
 "arp 300 0.28"
 "sub 300 0.28"
 "dim 18333 0.5" "swish 18333 0.24"
 "lift 40333 0.44" "swish 40333 0.26"
)
inp=""; fc=""; lab=""; n=${#rows[@]}
...
fc+="[mx]volume=3.0,acompressor=threshold=-20dB:ratio=3:attack=14:release=200,loudnorm=I=-16:TP=-1.5:LRA=11,alimiter=limit=0.97,lowpass=f=15000,afade=t=out:st=50.7:d=0.6,atrim=0:51.33,aformat=channel_layouts=stereo[out]"

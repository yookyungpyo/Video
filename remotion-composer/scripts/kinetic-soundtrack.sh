...
genf air      "(random(0)*2-1)" 24 "lowpass=f=520, lowpass=f=520, tremolo=f=0.1:d=0.6"
genf subpulse "sin(2*PI*98*t)*exp(-2*t)*(1-exp(-8*t))" 2.2 "lowpass=f=300, aecho=0.8:0.85:160:0.3"
genf whoosh   "(random(0)*2-1)*exp(-22*(t-0.22)^2)" 0.5 "bandpass=f=900:width_type=h:w=1300, lowpass=f=2000, aecho=0.8:0.85:90|170:0.3|0.2"
...
rows=(
 "drone 0 0.16" "air 0 0.05"
 "subpulse 700 0.12"
 "whoosh 4200 0.18" "whoosh 4700 0.18" "whoosh 5200 0.18"
 "riser 6400 0.26"
 "boom 7570 1.0"
 "whoosh 11300 0.14" "whoosh 12000 0.14"
 "swell 13700 0.22" "swell 15300 0.26" "swell 17400 0.16" "swell 20900 0.22"
)
inputs=""; fc=""; mixlabels=""; n=${#rows[@]}
for i in "${!rows[@]}"; do
  set -- ${rows[$i]}; f=$1; d=$2; v=$3
  inputs+=" -i $AD/$f.wav"
  if [ "$f" = "drone" ] || [ "$f" = "air" ]; then
    fc+="[$i]adelay=$d:all=1,volume=$v,afade=t=in:st=0:d=2[a$i];"
  else
...
fc+="[mx]acompressor=threshold=-18dB:ratio=3:attack=20:release=250,alimiter=limit=0.95,lowpass=f=15000,volume=1.25,afade=t=out:st=23.2:d=0.8,atrim=0:24,aformat=channel_layouts=stereo[out]"

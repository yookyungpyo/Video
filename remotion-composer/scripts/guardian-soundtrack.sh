#!/usr/bin/env bash
# Soft clay soundtrack for the "보이지 않는 노력 / 막은 사람" 5-card video (~19.17s, 575f@30fps).
# Cuts (frames 110/240/350/460) = 3.67 / 8.00 / 11.67 / 15.33 s.
# Usage: bash scripts/guardian-soundtrack.sh <in.mp4> <out.mp4>
set -euo pipefail
IN="${1:-../projects/cardnews/guardian-video.mp4}"; OUT="${2:-../projects/cardnews/guardian-video-sound.mp4}"
...
rows=(
 ...
 "pop 3820 0.34" "twinkle 4020 0.18" "twinkle 4250 0.18"
...
fc+="[mx]volume=3.6,acompressor=threshold=-20dB:ratio=3:attack=14:release=200,loudnorm=I=-16:TP=-1.5:LRA=11,alimiter=limit=0.97,lowpass=f=15000,afade=t=out:st=18.6:d=0.5,atrim=0:19.05,aformat=channel_layouts=stereo[out]"

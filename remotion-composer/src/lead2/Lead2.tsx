...
export const Cover: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={BLUE} b={LAV} bare={bare}>
    <Pill x={540} y={225} text="리더십" color={LAV} delay={2} />
    <div style={{ ...centerText(350, 104, INK), textShadow: "3px 6px 0 rgba(120,110,160,0.12)" }}>난 사람을</div>
    <div style={{ ...centerText(490, 100, CORAL), textShadow: "3px 6px 0 rgba(120,110,160,0.12)" }}>설득하지 않아</div>
    <div style={{ ...centerText(645, 50, INK, BODY), fontWeight: 900 }}>리더의 진짜 무기는 따로 있다</div>
    <Mascot x={540} y={1015} w={405} delay={4} floatAmp={16} />
...
const StatementCard: React.FC<{
  ...
  <CardFrame a={a} b={b} bare={bare}>
    ...
    <ClayCard x={540} y={615} w={920} bg="#FFFFFF" delay={6} float={6} pad="30px 40px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 84, color: bigColor, lineHeight: 1.05, textAlign: "center", whiteSpace: "nowrap" }}>{big}</div>
    </ClayCard>
...
export const Example: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    bare={bare}
    icon="crown"
    lead="말로 설득하는 대신"
    big="모범을 보여"
    bigColor={CORAL}
    detail={<>백 마디 말보다<br />한 번의 행동이 강하니까</>}
    tags={[{ t: "행동", c: CORAL }, { t: "솔선수범", c: BLUE }, { t: "꾸준함", c: MINT }]}
    a={YELLOW}
    b={CORAL}
  />
);

export const Follow: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    bare={bare}
    icon="converge"
    lead="리더가 먼저 움직이면"
    big="사람은 따라온다"
    bigColor={BLUE}
    detail={<>굳이 시키지 않아도<br />보고 배우니까</>}
    tags={[{ t: "신뢰", c: BLUE }, { t: "영향력", c: MINT }, { t: "자발성", c: LAV }]}
    a={BLUE}
    b={MINT}
  />
);

export const Confidence: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={CORAL} b={YELLOW} bare={bare}>
    <Pill x={540} y={300} text="내가 하는데," color={LAV} delay={2} />
    <div style={{ ...centerText(455, 140, CORAL), textShadow: "3px 6px 0 rgba(120,110,160,0.12)" }}>안 따라와?</div>
    <div style={{ ...centerText(690, 50, "#5b5570", BODY), fontWeight: 900 }}>솔선수범 앞엔 설득이 필요 없다</div>
    <Mascot x={540} y={1070} w={320} delay={6} floatAmp={15} />
...
export const Closing: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={BLUE} b={LAV} bare={bare}>
    ...
    <div style={{ ...centerText(430, 58, INK) }}>리더의 방식은</div>
    <ClayCard x={540} y={630} w={980} bg="#FFFFFF" delay={8} float={7} pad="26px 40px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 66, color: CORAL, lineHeight: 1.0, textAlign: "center", whiteSpace: "nowrap" }}>설득하지 말고, 보여줘라</div>
    </ClayCard>
    <div style={{ ...centerText(790, 44, "#5b5570", BODY), fontWeight: 700 }}>말이 아니라, 행동으로 이끈다</div>
    <Mascot x={540} y={1055} w={285} delay={6} floatAmp={14} />
    <div style={{ ...centerText(1300, 42, "#7a728e") }}>www.wylieax.com</div>
  </CardFrame>
);

...
export const Cover: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={LAV} b={BLUE} bare={bare}>
    <Pill x={540} y={225} text="SNS · 마음" color={LAV} delay={2} />
    <div style={{ ...centerText(350, 116, INK), textShadow: "3px 6px 0 rgba(120,110,160,0.12)" }}>SNS는</div>
    <div style={{ ...centerText(500, 100, CORAL), textShadow: "3px 6px 0 rgba(120,110,160,0.12)" }}>편집된 일부</div>
    <div style={{ ...centerText(655, 50, INK, BODY), fontWeight: 900 }}>그 사람의 전부가 아니야</div>
...
const StatementCard: React.FC<{
  ...
  <CardFrame a={a} b={b} bare={bare}>
    ...
    <ClayCard x={540} y={615} w={940} bg="#FFFFFF" delay={6} float={6} pad="30px 40px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 82, color: bigColor, lineHeight: 1.05, textAlign: "center", whiteSpace: "nowrap" }}>{big}</div>
    </ClayCard>
...
export const Truth: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    bare={bare}
    icon="heart"
    lead="우리가 보는 건"
    big="하이라이트뿐"
    bigColor={BLUE}
    detail={<>수많은 순간 중<br />가장 좋은 장면만 골랐을 뿐</>}
    tags={[{ t: "연출", c: BLUE }, { t: "순간", c: MINT }, { t: "일부", c: LAV }]}
    a={BLUE}
    b={MINT}
  />
);

export const Trap: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    bare={bare}
    icon="mask"
    lead="그런데 우리는"
    big="내 전부와 비교해"
    bigColor={CORAL}
    detail={<>남의 '편집본'과 내 '무편집본'<br />애초에 공정하지 않은 싸움</>}
    tags={[{ t: "과장", c: CORAL }, { t: "연출", c: BLUE }, { t: "일부", c: LAV }]}
    a={CORAL}
    b={YELLOW}
  />
);

export const Shift: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    bare={bare}
    icon="sun"
    lead="비교를 내려놓으면"
    big="내 삶이 보여"
    bigColor={MINT}
    detail={<>남의 한 장면 말고<br />내 전체를 사는 것</>}
    tags={[{ t: "지금", c: MINT }, { t: "나답게", c: BLUE }, { t: "충분함", c: YELLOW }]}
    a={MINT}
    b={BLUE}
  />
);
...
export const Closing: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={LAV} b={BLUE} bare={bare}>
    ...
    <div style={{ ...centerText(430, 58, INK) }}>그러니,</div>
    <ClayCard x={540} y={630} w={900} bg="#FFFFFF" delay={8} float={7} pad="26px 40px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 82, color: CORAL, lineHeight: 1.0, textAlign: "center", whiteSpace: "nowrap" }}>편집본과 싸우지 마</div>
    </ClayCard>
    <div style={{ ...centerText(795, 52, "#5b5570", BODY), fontWeight: 900 }}>넌 이미, 충분해</div>

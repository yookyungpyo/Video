...
const ICONS: Record<string, React.ReactNode> = {
  ...
  shield: (
    <>
      <path d="M50 18 L80 30 V54 C80 72 66 82 50 88 C34 82 20 72 20 54 V30 Z" fill="none" stroke="#fff" strokeWidth={8} strokeLinejoin="round" />
      <path d="M38 52 L47 62 L64 42" fill="none" stroke="#fff" strokeWidth={8} strokeLinecap="round" strokeLinejoin="round" />
    </>
  ),
  growth: (
    <>
      <path d="M20 82 L82 82" stroke="#fff" strokeWidth={8} strokeLinecap="round" />
      <rect x={28} y={58} width={13} height={22} rx={3} fill="#fff" />
      <rect x={44} y={44} width={13} height={36} rx={3} fill="#fff" />
      <rect x={60} y={28} width={13} height={52} rx={3} fill="#fff" />
      <path d="M24 40 L44 30 L60 34 L80 18 M80 18 L70 18 M80 18 L80 28" fill="none" stroke="#fff" strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" />
    </>
  ),
};
...
export const Cover: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={MINT} b={BLUE} bare={bare}>
    ...
    <div style={{ ...centerText(600, 46, INK, BODY), fontWeight: 900 }}>무엇이 다를까?</div>
    <Mascot x={540} y={1015} w={400} delay={4} floatAmp={16} />
...
const StatementCard: React.FC<{
  ...
  <CardFrame a={a} b={b} bare={bare}>
    ...
    <ClayCard x={540} y={615} w={960} bg="#FFFFFF" delay={6} float={6} pad="28px 36px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 78, color: bigColor, lineHeight: 1.05, textAlign: "center", whiteSpace: "nowrap" }}>{big}</div>
    </ClayCard>
...
export const Define: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    ...
    big="실수해도 안전한 느낌"
    bigColor={BLUE}
    detail={<>질문·실수·이견을<br />벌 받지 않고 말할 수 있는 상태</>}
    tags={[{ t: "신뢰", c: BLUE }, { t: "존중", c: MINT }, { t: "솔직함", c: YELLOW }]}
    a={BLUE}
    b={MINT}
  />
);

const colBox = (x: number, y: number, w: number, size: number, color: string, family = FONT): React.CSSProperties => ({
  position: "absolute",
  left: x - w / 2,
  top: y,
  width: w,
...
const VersusCard: React.FC<{
  ...
  <CardFrame a={a} b={b} bare={bare}>
    ...
    <div
      style={{
        position: "absolute", left: 540, top: 345, width: 108, height: 108, marginLeft: -54, marginTop: -54,
        borderRadius: "50%", background: LAV, boxShadow: claySh, display: "flex", alignItems: "center",
        justifyContent: "center", fontFamily: FONT, fontSize: 42, color: "#fff",
      }}
    >
      VS
    </div>
    <div style={{ position: "absolute", left: 539, top: 455, width: 3, height: 420, background: "rgba(120,110,160,0.28)", borderRadius: 2 }} />
    {rows.map((r, i) => {
      const y = 500 + i * 140;
      return (
        <React.Fragment key={i}>
          <div style={colBox(286, y, 440, 44, INK)}>{r.l}</div>
          <div style={colBox(794, y, 440, 44, INK)}>{r.r}</div>
        </React.Fragment>
      );
    })}
    <Mascot x={540} y={1140} w={175} delay={10} floatAmp={10} />
...
export const Result: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    ...
    big="안전한 팀이 이긴다"
    bigColor={MINT}
    detail={<>심리적 안정감이 높은 팀일수록<br />더 배우고 더 혁신한다</>}
    tags={[{ t: "학습", c: MINT }, { t: "몰입", c: BLUE }, { t: "혁신", c: CORAL }]}
    a={MINT}
    b={BLUE}
  />
);
...
export const Closing: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={BLUE} b={MINT} bare={bare}>
    ...
    <ClayCard x={540} y={620} w={860} bg="#FFFFFF" delay={8} float={7} pad="24px 40px" radius={44}>
      <div style={{ fontFamily: FONT, fontSize: 92, color: CORAL, lineHeight: 1.0, textAlign: "center", whiteSpace: "nowrap" }}>말해야 성장한다</div>
    </ClayCard>

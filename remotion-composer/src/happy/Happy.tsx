...
export const Cover: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={YELLOW} b={CORAL} bare={bare}>
    ...
    <div style={{ ...centerText(645, 50, INK, BODY), fontWeight: 900 }}>역치는 낮게, 크기는 작게</div>
...
export const Threshold: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    ...
    big="역치를 낮게"
    bigColor={CORAL}
    detail={<>작은 것에도 쉽게 웃으면<br />하루에도 여러 번 행복해</>}
    tags={[{ t: "커피 한잔", c: CORAL }, { t: "산책", c: MINT }, { t: "햇살", c: YELLOW }]}
    a={YELLOW}
    b={MINT}
  />
);
...
export const Size: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <StatementCard
    ...
    big="크기를 작게"
    bigColor={BLUE}
    detail={<>큰 행복 하나를 기다리기보다<br />작은 행복 여럿을 누리기</>}
    tags={[{ t: "소소함", c: BLUE }, { t: "지금", c: MINT }, { t: "자주", c: LAV }]}
    a={BLUE}
    b={MINT}
  />
);
...
export const Effect: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={MINT} b={BLUE} bare={bare}>
    ...
    <div style={{ ...centerText(730, 48, "#5b5570", BODY), fontWeight: 900 }}>특별한 날이 아니어도 충분히</div>
    <Mascot x={540} y={1075} w={300} delay={6} floatAmp={14} />
...
export const Closing: React.FC<{ bare?: boolean }> = ({ bare }) => (
  <CardFrame a={YELLOW} b={CORAL} bare={bare}>
    ...
    <ClayCard x={540} y={630} w={960} bg="#FFFFFF" delay={8} float={7} pad="26px 40px" radius={46}>
      <div style={{ fontFamily: FONT, fontSize: 68, color: CORAL, lineHeight: 1.0, textAlign: "center", whiteSpace: "nowrap" }}>낮게, 작게, 자주 행복하자</div>
    </ClayCard>
    <div style={{ ...centerText(790, 42, "#5b5570", BODY), fontWeight: 700 }}>오늘도 너의 하루에 작은 행복이 많기를</div>

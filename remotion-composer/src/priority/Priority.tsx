...
const ICONS: Record<string, (c: string) => React.ReactNode> = {
  ...
  target: (c) => (
    <>
      ...
      <circle cx={50} cy={50} r={24} fill="none" stroke={c} strokeWidth={8} />
      <circle cx={50} cy={50} r={9} fill={c} />
    </>
  ),
  trash: (c) => (
    <>
      <path d="M22 30 L78 30 L72 92 L28 92 Z" fill={c} stroke={INK} strokeWidth={5} strokeLinejoin="round" />
      <path d="M16 28 L84 28" stroke={INK} strokeWidth={8} strokeLinecap="round" />
      <path d="M38 24 L62 24" stroke={INK} strokeWidth={8} strokeLinecap="round" />
      <path d="M40 44 L42 80 M58 44 L56 80" stroke={INK} strokeWidth={5} strokeLinecap="round" />
    </>
  ),
  converge: (c) => (
    <>
      <circle cx={50} cy={50} r={11} fill={c} />
      <path d="M50 8 L50 30 M44 24 L50 30 L56 24" fill="none" stroke={c} strokeWidth={7} strokeLinecap="round" strokeLinejoin="round" />
      <path d="M92 50 L70 50 M76 44 L70 50 L76 56" fill="none" stroke={c} strokeWidth={7} strokeLinecap="round" strokeLinejoin="round" />
      <path d="M50 92 L50 70 M44 76 L50 70 L56 76" fill="none" stroke={c} strokeWidth={7} strokeLinecap="round" strokeLinejoin="round" />
      <path d="M8 50 L30 50 M24 44 L30 50 L24 56" fill="none" stroke={c} strokeWidth={7} strokeLinecap="round" strokeLinejoin="round" />
    </>
  ),
};
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 22 }}>
      <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} style={{ flexShrink: 0 }}>
        <path
          d={`M ${s / 2} 2 L ${s * 0.61} ${s * 0.39} L ${s - 2} ${s / 2} L ${s * 0.61} ${s * 0.61} L ${s / 2} ${s - 2} L ${s * 0.39} ${s * 0.61} L 2 ${s / 2} L ${s * 0.39} ${s * 0.39} Z`}
          fill={YELLOW}
          stroke={INK}
          strokeWidth={3}
          strokeLinejoin="round"
      ...
      <div style={{ fontFamily: BODY, fontWeight: 900, fontSize: 40, color: INK, whiteSpace: "nowrap" }}>{label}</div>
    </div>
  );
};
...
const Hook: React.FC = () => (
  <AbsoluteFill>
    <Tape x={540} y={452} w={330} rot={-5} />
    <Label x={540} y={452} text="전부 다 중요하다고?" size={62} color={INK} delay={2} rot={-2} />
    <BigWord x={540} y={620} text="모든 게 중요하면" size={98} delay={8} seed={4} rot={-2} />
    <BigWord x={540} y={760} text="= 아무것도 아니다" size={104} color={RED} delay={16} seed={5} rot={1} />
    <ScribbleCircle x={640} y={760} w={360} h={150} color={PINK} delay={26} />
    <Label x={540} y={930} text="그래서, 고르는 법" size={60} color={BLUE} delay={32} />
    {/* checklist note: every line starred = nothing stands out */}
    <Card x={540} y={1290} rot={-4} bg="#fff" seed={7} delay={26} pad="34px 56px">
      <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
        <StarRow label="해야 할 일" />
        <StarRow label="급한 일" />
        <StarRow label="중요한 일" />
        <StarRow label="사소한 일" />
      </div>
...
const Close: React.FC = () => {
  ...
  return (
    <AbsoluteFill>
      <Label x={540} y={425} text="딱 하나만 기억하세요" size={66} color={INK} delay={2} />
      <BigWord x={540} y={565} text="선택과 집중" size={130} color={RED} delay={8} seed={6} />
      <ScribbleCircle x={540} y={565} w={520} h={210} color={PINK} delay={26} />
      {cards.map((c, i) => (
        <Card key={i} x={250 + i * 290} y={760} rot={i % 2 ? 4 : -4} bg={c.c} seed={i + 20} delay={16 + i * 6} pad="18px 26px">
          <div style={{ fontFamily: DISPLAY, fontSize: 50, color: "#fff" }}>{c.t}</div>
        </Card>
      ))}
      {/* conclusion */}
      <div
      ...
      >
        ...
        <br />
        <span style={{ color: RED }}>'아무것도 안 하겠다'</span>와 같다
      </div>
      <Label x={540} y={1120} text="당신의 1순위는 무엇인가요?" size={62} color="#3f372c" delay={30} />
